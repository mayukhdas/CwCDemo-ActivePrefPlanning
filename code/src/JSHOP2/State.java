...
public class State
{
  ...
  */
  ...
  {
	  State cloneState = new State(this.atoms.length,this.axioms);
	  for(int i=0;i<atoms.length;i++)
		  cloneState.atoms[i] = (Vector<Term>) this.atoms[i].clone();
	  for(int i=0;i<this.protections.length;i++)
		  cloneState.protections[i] = (Vector<NumberedPredicate>) this.protections[i].clone();
	  cloneState.axioms = new Axiom[this.axioms.length][this.axioms[0].length];
	  for(int i=0;i<this.axioms.length;i++)
		  for(int j = 0;j<this.axioms[0].length;j++)
			  cloneState.axioms[i][j] = this.axioms[i][j];
	  return cloneState;
